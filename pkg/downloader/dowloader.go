...
// Home is the interface to fetch repository files and the cache index
type Home interface {
	CacheIndex(string) string
	RepositoryFile() string
}

// Downloader handles downloading a managed archived file (chart, pack).
//
// It is capable of performing verifications on archives as well.
type Downloader struct {
	...
	Keyring string
	// Home is the home from the consumer ($HELM_HOME, $DRAFT_HOME).
	Home Home
...
func (c *Downloader) DownloadTo(ref, version, dest string) (string, *provenance.Verification, error) {
	u, g, err := c.ResolveVersion(ref, version)
	if err != nil {
		return "", nil, err
	}
	...
	if c.Verify > VerifyNever {
		...
		if c.Verify != VerifyLater {
			ver, err = VerifyFile(destfile, c.Keyring)
			if err != nil {
...
// ResolveVersion resolves a chart reference to a URL.
//
...
func (c *Downloader) ResolveVersion(ref, version string) (*url.URL, getter.Getter, error) {
	u, err := url.Parse(ref)
	if err != nil {
	...
	if u.IsAbs() && len(u.Host) > 0 && len(u.Path) > 0 {
		...
		r, err := repo.NewRepository(rc, c.Getters)
	...
	repoName := p[0]
	chartName := p[1]
	rc, err := pickRepositoryConfigByName(repoName, rf.Repositories)
	if err != nil {
		return u, nil, err
	}
	...
	i, err := repo.LoadIndexFile(c.Home.CacheIndex(r.Config.Name))
	if err != nil {
...
// VerifyFile takes a path to a archive file and a keyring, and verifies the archive.
//
// It assumes that an archive file is accompanied by a provenance file whose
// name is the archive file name plus the ".prov" extension.
func VerifyFile(path string, keyring string) (*provenance.Verification, error) {
	// For now, error out if it's not a tar file.
	if fi, err := os.Stat(path); err != nil {
		return nil, err
	} else if fi.IsDir() {
		return nil, errors.New("unpacked files cannot be verified")
	} else if !isTar(path) {
		return nil, errors.New("file must be a tgz file")
	}
...
func pickRepositoryConfigByName(name string, cfgs []*repo.Entry) (*repo.Entry, error) {
	for _, rc := range cfgs {
		if rc.Name == name {
			if rc.URL == "" {
...
func (c *Downloader) scanReposForURL(u string, rf *repo.RepoFile) (*repo.Entry, error) {
