...
func NewTempServer(glob string) (*Server, string, error) {
	tdir, err := ioutil.TempDir("", "draft-repotest-")
	if err != nil {
		return nil, tdir, err
	}
	srv := NewServer(tdir)
	...
	if glob != "" {
		if _, err := srv.Copy(glob); err != nil {
			srv.Stop()
			return srv, tdir, err
		}
	}
...
//
// Use Copy to move file into the repository and then index them
// for service.
func NewServer(docroot string) *Server {
	...
	if err := setTestingRepository(draftpath.NewHome(docroot), "test", srv.URL()); err != nil {
		panic(err)
	}
	return srv
}
...
// Copy takes a glob expression and copies those files to the server root.
func (s *Server) Copy(origin string) ([]string, error) {
...
func setTestingRepository(home draftpath.Home, name, url string) error {
	r := repo.NewRepoFile()
	r.Add(&repo.Entry{
		Name:  name,
		URL:   url,
