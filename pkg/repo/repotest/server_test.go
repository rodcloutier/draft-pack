...
func TestServer(t *testing.T) {
	docroot, err := ioutil.TempDir("", "draft-repotest-")
	if err != nil {
		t.Fatal(err)
	}
	...
	if filepath.Base(c[0]) != "examplepack.tgz" {
		t.Errorf("Unexpected pack: %s", c[0])
	}

	res, err := http.Get(srv.URL() + "/examplepack.tgz")
	if err != nil {
		t.Error(err)
		return
	}
...
func TestNewTempServer(t *testing.T) {
	srv, tdir, err := NewTempServer("testdata/examplepack.tgz")
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	res, err := http.Head(srv.URL() + "/examplepack.tgz")
	if err != nil {
		t.Error(err)
	}
