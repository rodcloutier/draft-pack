...
to find matches.
*/
// From k8s.io/helm/cmd/helm/search/search.go
...
type Result struct {
	Name  string
	Score int
	Pack  *repo.PackVersion
}

// Index is a searchable index of pack information.
type Index struct {
	lines map[string]string
	packs map[string]*repo.PackVersion
}
...
func NewIndex() *Index {
	return &Index{lines: map[string]string{}, packs: map[string]*repo.PackVersion{}}
}
...
func (i *Index) AddRepo(rname string, ind *repo.IndexFile, all bool) {
	...
	for name, ref := range ind.Entries {
		if len(ref) == 0 {
			// Skip pack names that have zero releases.
			continue
		}
		...
		for _, rr := range ref {
			...
			i.packs[versionedName] = rr
		}
	}
}

// All returns all packs in the index as if they were search results.
//
// Each will be given a score of 0.
func (i *Index) All() []*Result {
	res := make([]*Result, len(i.packs))
	j := 0
	for name, pack := range i.packs {
...
func (i *Index) SearchLiteral(term string, threshold int) []*Result {
	...
	for k, v := range i.lines {
		...
		if score := i.calcScore(res, v); res != -1 && score < threshold {
			...
			buf = append(buf, &Result{Name: parts[0], Score: score, Pack: i.packs[k]})
		}
	}
	return buf
}
...
// Pack returns the PackVersion for a particular name.
func (i *Index) Pack(name string) (*repo.PackVersion, error) {
	c, ok := i.packs[name]
	if !ok {
		return nil, errors.New("no such pack")
	}
	return c, nil
}
...
func (s scoreSorter) Less(a, b int) bool {
	...
	if first.Name == second.Name {
		v1, err := semver.NewVersion(first.Pack.Version)
		if err != nil {
			return true
		}
		v2, err := semver.NewVersion(second.Pack.Version)
		if err != nil {
			return true
		}
		// Sort so that the newest pack is higher than the oldest pack. This is
...
func indstr(name string, ref *repo.PackVersion) string {
	i := ref.Name + sep + name + "/" + ref.Name + sep +
		ref.Description + sep + strings.Join(ref.Keywords, " ")
	return strings.ToLower(i)
}

func BuildIndex(home draftpath.Home, versions bool) (*Index, error) {
	...
	for _, re := range rf.Repositories {
		...
		if err != nil {
			fmt.Printf("WARNING: Repo %q is corrupt or missing. Try 'draft packs repo update'.", n)
			continue
		}

		i.AddRepo(n, ind, versions)
	}
	return i, nil
}
