...
func TestSortScore(t *testing.T) {
	in := []*Result{
		{Name: "bbb", Score: 0, Pack: &repo.PackVersion{Metadata: &pack.Metadata{Version: "1.2.3"}}},
		{Name: "aaa", Score: 5},
		{Name: "abb", Score: 5},
		{Name: "aab", Score: 0},
		{Name: "bab", Score: 5},
		{Name: "ver", Score: 5, Pack: &repo.PackVersion{Metadata: &pack.Metadata{Version: "1.2.4"}}},
		{Name: "ver", Score: 5, Pack: &repo.PackVersion{Metadata: &pack.Metadata{Version: "1.2.3"}}},
	}
	...
	if in[5].Pack.Version != "1.2.4" {
		t.Errorf("Expected 1.2.4, got %s", in[5].Pack.Version)
	}
	if in[6].Pack.Version != "1.2.3" {
		t.Error("Expected 1.2.3 to be last")
	}
}

var indexfileEntries = map[string]repo.PackVersions{
	"niña": {
		{
			URLs: []string{"http://example.com/packs/nina-0.1.0.tgz"},
	...
	"pinta": {
		{
			URLs: []string{"http://example.com/packs/pinta-0.1.0.tgz"},
			Metadata: &pack.Metadata{
				Name:        "pinta",
				Version:     "0.1.0",
	...
	"santa-maria": {
		{
			URLs: []string{"http://example.com/packs/santa-maria-1.2.3.tgz"},
			Metadata: &pack.Metadata{
				Name:        "santa-maria",
				Version:     "1.2.3",
		...
		{
			URLs: []string{"http://example.com/packs/santa-maria-1.2.2.tgz"},
			Metadata: &pack.Metadata{
				Name:        "santa-maria",
				Version:     "1.2.2",
				Description: "Three boat",
...
func loadTestIndex(t *testing.T, all bool) *Index {
	...
	i.AddRepo("ztesting", &repo.IndexFile{Entries: map[string]repo.PackVersions{
		"pinta": {
			{
				URLs: []string{"http://example.com/packs/pinta-2.0.0.tgz"},
				Metadata: &pack.Metadata{
					Name:        "pinta",
					Version:     "2.0.0",
...
func TestAddRepo_Sort(t *testing.T) {
	...
	ch := sr[0]
	expect := "1.2.3"
	if ch.Pack.Version != expect {
		t.Errorf("Expected %q, got %q", expect, ch.Pack.Version)
	}
}
...
func TestSearchByName(t *testing.T) {
	...
	for _, tt := range tests {
		...
		packs, err := i.Search(tt.query, 100, tt.regexp)
		if err != nil {
			if tt.fail {
...
func TestSearchByNameAll(t *testing.T) {
	...
	if len(cs) != 2 {
		t.Errorf("expected 2 packs, got %d", len(cs))
	}
}
